...
class KNearestNeighbor(object):
    ...
    # Step 2: ||X_train||^2 for all training samples (shape: [num_train,])
        X_train_square = np.sum(self.X_train ** 2, axis=1)
    
    # Step 3: X @ X_train.T gives inner product between test and train samples
        cross_term = X @ self.X_train.T
    
    # Step 4: Apply formula to get distance matrix
    ...
    def predict_labels(self, dists, k=1):
        ...
        for i in range(num_test):
            ...
            y_pred[i] = labels[max_count_indices].min()
